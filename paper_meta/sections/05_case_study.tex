...
\begin{table}[t]
  ...
  \begin{tabular}{l r}
    ...
    \midrule
    \multicolumn{2}{l}{\emph{LiuLab branch (\texttt{tools/cursor\_manager}) --- integer commit/insertion totals depend on \texttt{HEAD}}} \\
    \multicolumn{2}{l}{\footnotesize (reproduce with the \texttt{git log} one-liners in the table note below)} \\
  ...
  \medskip
  \footnotesize
  \noindent\emph{Reproduce LOC/smoke on OSS \texttt{main}:} \texttt{wc -l mgr lib/*.py sentinel.py scripts/*.py scripts/*.sh *.sh docs/sub\_agents\_contract.md}; \texttt{bash scripts/smoke\_test\_sub\_agents.sh}. \emph{Git counters:} on the public mirror, from repo root, \texttt{git log --oneline main | wc -l}; on the LiuLab monorepo with \texttt{HEAD} on \texttt{feature/autoresearch-sub-agents}, \texttt{git log --oneline HEAD -- tools/cursor\_manager | wc -l} and \texttt{git log --shortstat HEAD -- tools/cursor\_manager} (sum insertion fields).
