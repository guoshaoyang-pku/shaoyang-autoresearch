...
\If{paper-claim file (abstract / intro / conclusion) touched \textbf{or} diff $\geq 2$ files}
    \State \texttt{mgr review-sim <wid>} \Comment{cross-model NeurIPS-reviewer judgement}
\EndIf
...
\(770\) lines (out of \(5{,}969\) total LOC for the system in Python
plus Bash, measured by \texttt{wc -l mgr lib/*.py sentinel.py
scripts/\{*.py,*.sh\} *.sh}); this
...
\S\ref{sec:case} mines for empirical claims.
...
(\texttt{docs/sub\_agents\_contract.md}, 595 lines) that fully specifies,
